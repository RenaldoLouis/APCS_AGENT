...
- **Ticketing & Seat Booking Flow:** Whenever working on or modifying logic related to seat booking, ticketing flows, or venue settings, you **MUST** refer to `docs/SEAT_BOOKING_FLOW.md` to understand the architecture and interactions between `SystemSettings`, `VenueSettings`, `OrchestraSettings`, and `PublicTicketBookingPage`. You must also ensure that `docs/TICKETING_SYSTEM_GUIDE.md` is updated and kept in sync with these architectural details to maintain consistency between the technical flow and internal staff guidelines.
