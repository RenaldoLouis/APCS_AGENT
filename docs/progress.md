...
## 2026-09-23 — Video duration visibility in scoring and performer sessions

- Show each registrant's `videoDuration` in Scoring Recap and on performer assignment cards.
- Show the sum of known assigned video durations beside each session's scheduled length, with an overrun warning and an unknown-duration count. The warning does not block assignment saves.

## 2026-09-23 — Public and eligible-winner Paper.id staging payment rehearsal

- Reproduced that a flat production-style Invoice Paid callback with `invoice.amount` failed public-booking fulfillment, which previously read only staging `invoice.total_amount`. The shared paid transaction now accepts either total field and rejects mismatched or conflicting totals. Both callback routes already normalize nested staging and flat production envelopes.
- Offline regression covered public and eligible-winner checkout contacts, staging and production amount forms, and conflicting totals. The complete audit suite passed: **101 tests, 0 failures**; backend syntax checks and `git diff --check` passed. The running local `node index.js` process was started before the production-form edit, so it has not yet loaded that change.
