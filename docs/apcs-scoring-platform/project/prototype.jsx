...
const SEED = [
  ...
  { id: 'p3',  n: 'Harmoni Ensemble',  t: 'Group',      c: 'Professional', song: 'Bohemian Rhapsody',   d: '6:45', s: 'Pending',  sub: '07 May 2025, 11:08', u: null,                 data: null, members: ['Adi Wirawan','Citra Lestari','Bayu Saputra','Maya Indah'] },
  ...
  { id: 'p5',  n: 'Gema Suara Group',  t: 'Group',      c: 'Junior',       song: 'Canon in D',          d: '4:33', s: 'Pending',  sub: '08 May 2025, 14:30', u: null,                 data: null, members: ['Rini Pertiwi','Joko Saputra','Lina Hidayat'] },
  ...
  { id: 'p8',  n: 'Nada Bersama',      t: 'Group',      c: 'Primary',      song: 'You Are My Sunshine', d: '3:55', s: 'Pending',  sub: '06 May 2025, 13:09', u: null,                 data: null, members: ['Sinta Maharani','Eko Prasetyo','Dewi Anggraini'] },
  { id: 'p9',  n: 'Rangga Adi',        t: 'Individual', c: 'Junior',       song: 'Prelude in C',        d: '2:48', s: 'Pending',  sub: '08 May 2025, 18:00', u: null,                 data: null },
  { id: 'p10', n: 'Suara Senja',       t: 'Group',      c: 'Others',       song: 'Hallelujah',          d: '5:30', s: 'Pending',  sub: '07 May 2025, 21:14', u: null,                 data: null, members: ['Bagus Wicaksono','Nadia Putri','Reza Mahendra'] },
  ...
  { id: 'p14', n: 'Lentera Choir',     t: 'Group',      c: 'Junior',       song: 'Lean on Me',          d: '3:42', s: 'Pending',  sub: '08 May 2025, 08:21', u: null,                 data: null, members: ['Ayu Lestari','Doni Saputra','Ines Hartono','Galang Pratama'] },
...
const Login = ({ onSignIn }) => {
  ...
  return (
    <div className="screen" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, minHeight: '100vh' }}>
      <div style={{ width: 380 }}>
        <div style={{ textAlign: 'center', marginBottom: 22 }}>
          <div style={{
            width: 52, height: 52, background: 'var(--ink)', color: 'var(--paper)',
            borderRadius: 12,
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em',
          }}>A</div>
          <div style={{ fontSize: 22, fontWeight: 600, marginTop: 14, letterSpacing: '-0.02em' }}>
            APCS Scoring Platform
          </div>
          <div className="label" style={{ marginTop: 6 }}>teacher portal · sign in</div>
        </div>
        ...
        <form className="box" style={{ padding: 22 }} onSubmit={submit}>
          ...
          <div className="row between" style={{ marginTop: 14, marginBottom: 6 }}>
            ...
            <a className="label" style={{ textTransform: 'none', letterSpacing: 0, fontSize: 12, cursor: 'pointer', textDecoration: 'underline' }}>
              Forgot password?
            </a>
          </div>
...
const Dashboard = ({ participants, onOpen, onSignOut }) => {
      ...
      <div style={{ padding: '20px 28px', maxWidth: 1400, width: '100%', margin: '0 auto', flex: 1 }}>
        ...
        <div className="row between" style={{ marginBottom: 18 }}>
          ...
          <div className="row" style={{ gap: 8 }}>
            <span className="pill"><Icon id="clock" size={11} /> Deadline 18 May</span>
          </div>
        </div>
...
// ============================================================
// MEDIA MODAL (sheet music / YouTube)
// ============================================================
const MediaModal = ({ kind, participant, onClose }) => {
  ...
  const title    = isSheet ? 'Sheet music' : 'Performance video';
  ...
  return (
    <div className="ovl" onClick={onClose}>
      <div
        ...
        {/* Footer */}
        <div className="row between" style={{ padding: '12px 18px', borderTop: '1px solid var(--rule)', background: 'var(--paper-3)' }}>
          <div className="label" style={{ textTransform: 'none', letterSpacing: 0, fontSize: 12 }}>
            {isSheet ? 'Provided by the participant.' : 'Hosted on YouTube · uploaded by participant.'}
          </div>
          <div className="row" style={{ gap: 8 }}>
            {isSheet
              ? <button className="btn sm"><Icon id="pdf" size={12} /> Open PDF</button>
              : <button className="btn sm"><Icon id="out" size={12} /> Open in YouTube</button>}
...
const AssessmentForm = ({ participant, locked, pendingQueue, onBack, onSave, onNext }) => {
  // initial values: from saved data if present, else blank-ish defaults
  const init = participant.data || { score: 75, minus: 0, feedback: '', comments: '' };
  const [score, setScore]       = useState(init.score);
  const [minus, setMinus]       = useState(init.minus);
  ...
  useEffect(() => {
    const i = participant.data || { score: 75, minus: 0, feedback: '', comments: '' };
    setScore(i.score); setMinus(i.minus); setFeedback(i.feedback); setComments(i.comments);
  }, [participant.id]);

  const final = Math.max(0, score - minus);
  const dirty = !locked && (
    score !== init.score || minus !== init.minus ||
    ...
    <div className="screen" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="row between" style={{ padding: '14px 28px', borderBottom: '1px solid var(--rule)', background: 'var(--paper-3)', gap: 18, flexWrap: 'wrap' }}>
        ...
        <div className="row" style={{ gap: 16, fontSize: 13, color: 'var(--ink-2)', flexWrap: 'wrap' }}>
          ...
          <a className="link" onClick={() => setMedia('video')}><Icon id="play" size={13} /> Watch on YouTube</a>
      ...
      <div style={{ padding: '24px 32px', maxWidth: 980, margin: '0 auto', width: '100%', flex: 1 }}>
        ...
        <div className="box" style={{ padding: 24 }}>
          <div className="row" style={{ gap: 32, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 200px' }}>
              ...
              <div className="row" style={{ alignItems: 'center', gap: 10, marginTop: 8 }}>
                <input
                  ...
                  style={{ width: 110, fontSize: 16, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}
                />
            ...
            <div style={{ flex: '1 1 200px' }}>
              <div className="label">
                Minus points <span style={{ textTransform: 'none', letterSpacing: 0, color: 'var(--ink-4)', marginLeft: 4 }}>· deduction</span>
              </div>
              <div className="row" style={{ gap: 8, marginTop: 8 }}>
                <button className="btn icon" onClick={() => setMinus(Math.max(0, minus - 1))} disabled={locked || minus <= 0}>
                ...
                <input
                  ...
                  onChange={e => {
                    const raw = e.target.value;
                    if (raw === '') { setMinus(0); return; }
                    const v = parseInt(raw, 10);
                    if (!isNaN(v)) setMinus(Math.max(0, Math.min(50, v)));
                  }}
                  style={{ width: 80, textAlign: 'center', fontSize: 16, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}
                />
                <button className="btn icon" onClick={() => setMinus(Math.min(50, minus + 1))} disabled={locked}>
                  <Icon id="plus" size={14} />
                </button>
              </div>
            </div>

            <div style={{ flex: '0 0 auto', textAlign: 'right', paddingLeft: 24, borderLeft: '1px solid var(--rule)', minWidth: 160 }}>
              <div className="label">Final score</div>
              <div style={{ marginTop: 4 }}>
                <span style={{ fontSize: 44, fontWeight: 600, lineHeight: 1, letterSpacing: '-0.03em' }}>
                  <span className="hi">{final}</span>
                </span>
              </div>
              <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 4, fontFamily: 'var(--type)' }}>
                {score} − {minus} = {final}
              </div>
            </div>
          </div>
        ...
        <div className="row" style={{ gap: 14, marginTop: 14, alignItems: 'stretch' }}>
          ...
          <div className="box grow" style={{ padding: 16 }}>
            ...
            <textarea
              ...
              placeholder="Anything else for the panel…"
              style={{ minHeight: 100, border: 'none', padding: 0, background: 'transparent' }}
            />
          </div>
        </div>
        ...
        <div className="row between box" style={{ padding: '12px 16px', marginTop: 16, background: 'var(--paper-2)' }}>
          ...
          <div className="row" style={{ gap: 10 }}>
            ...
            {!locked && (
              <button className="btn primary" onClick={() => onSave({ score, minus, final, feedback, comments })}>
  ...
  <div className="ovl">
    <div className="modal">
      ...
      <div style={{ fontSize: 13, color: 'var(--ink-2)', marginTop: 6, lineHeight: 1.5 }}>
        ...
        {score != null && <> Final score: <b>{score}</b>.</>}
      </div>
...
const App = () => {
  ...
  const save = ({ score, minus, final, feedback, comments }) => {
    setParticipants(prev => prev.map(p =>
      p.id === activeId
        ? { ...p, s: 'Assessed', u: fmtNow(), data: { score, minus, feedback, comments } }
        : p
    ));
    setSuccess({ participantId: activeId, score: final });
  };
