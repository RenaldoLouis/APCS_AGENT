...
<a className="brand" onClick={onClick}>
    <img src="assets/apc-logo.png" alt="APC" className="brand-logo-img" />
  </a>;
  ...
  return (
    <div className="screen login">
      <div className="login-art">
        <img src="assets/apc-stage.png" alt="A Piano Concerto Series performance" />
      </div>
...
const PrintPage = ({ title, children }) => (
  <section className="print-page">
    <div className="print-page-label">{title}</div>
...
const PrintApp = () => {
  const pendingForEdit   = SEED.find((p) => p.id === 'p1');  // Andi Pratama — pending
  const assessedForView  = SEED.find((p) => p.id === 'p2');  // Bela Nusantara — assessed
  const pendingIds = SEED.filter((p) => p.s === 'Pending').map((p) => p.id);
  const queueForEdit = {
    position: pendingIds.indexOf(pendingForEdit.id) + 1,
    total: pendingIds.length,
  };

  return (
    <div className="print-root">
      <PrintPage title="01 · Login">
        <Login onSignIn={noop} />
      </PrintPage>

      <PrintPage title="02 · Dashboard — normal">
        <Dashboard participants={SEED} onOpen={noop} onSignOut={noop} deadline="normal" />
      </PrintPage>

      <PrintPage title="03 · Dashboard — deadline H−7 warning">
        <Dashboard participants={SEED} onOpen={noop} onSignOut={noop} deadline="h7" />
      </PrintPage>

      <PrintPage title="04 · Dashboard — deadline H−1 warning">
        <Dashboard participants={SEED} onOpen={noop} onSignOut={noop} deadline="h1" />
      </PrintPage>

      <PrintPage title="05 · Assessment form — editing (pending)">
        <AssessmentForm
          participant={pendingForEdit}
          locked={false}
          pendingQueue={queueForEdit}
          onBack={noop} onSave={noop} onNext={noop} />
      </PrintPage>

      <PrintPage title="06 · Assessment form — view-only (assessed)">
        <AssessmentForm
          participant={assessedForView}
          locked={true}
          pendingQueue={null}
          onBack={noop} onSave={noop} onNext={noop} />
      </PrintPage>

      <PrintPage title="07 · Save success">
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
          ...
          <div style={{ position: 'absolute', inset: 0 }}>
            <SuccessModal participant={pendingForEdit} score={82} onBack={noop} />
          </div>
        </div>
      </PrintPage>
    </div>);
...
// Auto-print once fonts + paints settle
(async () => {
